...
RUN mkdir /svc
COPY --from=0 /root/target/universal/auththingie-0.2.1.tgz /svc
ADD scripts/generate_totp.sh /usr/local/bin/generate_totp
EXPOSE 9000
RUN cd /svc && yum install -y tar gzip && tar xvf auththingie-0.2.1.tgz && chmod +x /svc/auththingie-0.2.1/bin/auththingie
